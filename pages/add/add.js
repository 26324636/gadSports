      ...
      success(res) {
          ...
          if (that.data.timebushu.indexOf("小时") < 0) {
            ...
            if (res.data.url[i].type == "步数") {
              ...
              if (parseFloat(that.data.timebushu) < res.data.url[i].criterion) {
                wx.showToast({
                  title: '步数小于' + res.data.url[i].criterion + '，不能打卡！',
                  duration: 1000,
                  icon: "none"
                })
                flag1 = false;
            ...
            if (that.data.type == res.data.url[i].type) {
              if (parseFloat(that.data.timebushu) < res.data.url[i].criterion) {
                ...
                wx.showToast({
                  title: res.data.url[i].type + '小于' + res.data.url[i].criterion + '小时，不能打卡！',
                  duration: 1000,
                  icon: "none"
                })
                flag2 = false;
        ...
        if (that.data.timebushu.indexOf("小时") < 0) {
          ...
          if (flag1 == true && flag3 == true && flag4 == true) {
            wx.request({
              method: 'POST',
              url: url2 + '/punch/admin/sprotInsert', // 仅为示例，并非真实的接口地址
              data: {
                type: that.data.type,
  ...
  fenlei() {
    ...
    wx.request({
      ...
      success(res) {
        ...
        console.log(res.data.url[0].type)
                        ...
                        success(resRun) {
                          ...
                          wx.request({
                            ...
                            success: function (resDecrypt) {
                              ...
                              // console.log(util.toDate(runData.stepInfoList[da].timestamp));
                              that.setData({
